...
Malformed data for all column could be generated using the `--malformed-column-rate=0.1` build script option. With this mode the all the columns will have a percent of desired malformed data.
...
Not informed data could be generated using the `--not-informed` build script option. With this mode the not-informed schema will be generated by the way and every column will have a 0.1 rate of not-informed data.
